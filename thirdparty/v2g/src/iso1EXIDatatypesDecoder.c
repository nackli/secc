	...
	while(!done) {
		...
		case 257:
			...
			if (errn == 0) {
				switch(eventCode) {
				case 0:
//					printf("begin stream pos = %d, capacity = %d\n", *stream->pos, stream->capacity);
					body_start = (*stream->pos) * 8 - stream->capacity;
					errn = decode_iso1BodyType(stream, &iso1AnonType_V2G_Message->Body);
//					printf("end stream pos = %d, capacity = %d\n", *stream->pos, stream->capacity);
					body_end = (*stream->pos) * 8 - stream->capacity;
					grammarID = 3;
					break;
				default:
					errn = EXI_ERROR_UNKOWN_EVENT_CODE;
